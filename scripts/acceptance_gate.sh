...

{
  ...
  echo "| Summary | ${pass} PASS · ${fail} FAIL · ${man} MANUAL · ${skip} SKIP |"
  echo
  echo "> Release only when automated checks are green AND every MANUAL row is signed off."
  echo
  echo "## Checks"
  echo
  ...
  echo
  echo "| Role | Name | Date | Signature |"
  echo "| --- | --- | --- | --- |"
  echo "| Release engineer |  |  |  |"
  echo "| QA |  |  |  |"
  echo "| AI-safety reviewer (v0.7 RC+) |  |  |  |"
  echo
